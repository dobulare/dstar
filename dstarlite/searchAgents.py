...
class ReplanningSearchProblem(search.SearchProblem):
    ...
    def __init__(self, gameState, costFn=lambda x: 1, goal=(1, 1), start=None, warn=False, visualize=False):
        self.walls = gameState.getWalls()
        self.height = self.walls.height
        self.width = self.walls.width

        self.walls_explored = copy.deepcopy(self.walls)

        for row, column in self.walls_explored.asList():
            if row == 0 or column == 0 or column == self.width or row == self.height:
                continue
            else:
                self.walls_explored[row][column] = False
    ...
    def set_walls_explored(self, state):
        self.walls_explored[state[0]][state[1]] = True
    def get_walls_explored(self):
        return self.walls_explored

    def checkWall(self, state):
        return self.walls[state[0]][state[1]]

    def is_wall_found(self, state):
        return self.walls_explored[state[0]][state[1]]
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if 0 <= nextx < self.width and 0 <= nexty < self.height and not self.walls_explored[nextx][nexty]:
    ...
    def getCostOfActions(self, actions):
        ...
        for action in actions:
            ...
            if self.walls_explored[x][y]: return 999999
