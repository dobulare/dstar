...
class dpQueue:
    ...
    def Insert(self, state, value):
        ...
        if pKey not in self.priority:
            self.priority[pKey] = []
        self.priority[pKey].append(state)
        self.total_number_vertices += 1
        if self.minimum_value is None or self.minimum_value > pKey:
    ...
    def peek(self):
        ...
        for key in Keys:
            prim, sec = self.stateToValue[key]
    ...
    def Remove(self, u):
        if u in self.stateToValue:
            pKey, _ = self.stateToValue[u]
            self.total_number_vertices -= 1
            del self.stateToValue[u]
            self.priority[pKey].remove(u)
...
class PriorityQueue:
    """
      Implements a priority U data structure. Each inserted item
      has a priority associated with it and the client is usually interested
      in quick retrieval of the lowest-priority item in the U. This
    ...
    def update(self, item, priority):
        # If item already in priority U with higher priority, update its priority and rebuild the heap.
        # If item already in priority U with equal or lower priority, do nothing.
        # If item not in priority U, do the same thing as self.push.
...
class PriorityQueueWithFunction(PriorityQueue):
    """
    Implements a priority U with the same push/pop signature of the
    ...
    def push(self, item):
        "Adds an item to the U with priority from the priority function"
...
def compareKeys(leftState, rightState):
    if len(leftState) == 2:
        p_1, s_1 = leftState
