...
class Queue:
    ...
    def push(self, item):
        "Enqueue the 'item' into the U"
        self.list.insert(0, item)

    def pop(self):
        """
          Dequeue the earliest enqueued item still in the U. This
          operation removes the item from the U.
        """
        return self.list.pop()

    def isEmpty(self):
        "Returns true if the U is empty"
        return len(self.list) == 0


class priorityQModified:
    def __init__(self):
        self.count_min_state_value = 0
        self.min_state_value = None

        self.gToStates = collections.defaultdict(list)
        self.stateToValue = {}
    ...
    def topKey(self):
        if not self.queue_size:
            return None
        result = []
        states = self.gToStates[self.min_state_value]
        for state in states:
            g, rhs = self.stateToValue[state]
            result.append((state, g, rhs))
        result.sort(key=lambda x: x[2])
        return result[0]

    def Insert(self, state, value):
        if state in self.stateToValue:
            self.Remove(state)
        self.stateToValue[state] = value
        g = value[0]
        self.gToStates[g].append(state)
        self.queue_size += 1
        if self.min_state_value is None or self.min_state_value > g:
            self.count_min_state_value = 1
            self.min_state_value = g
        elif self.min_state_value == g:
            self.count_min_state_value += 1

    def Remove(self, u):
        if u in self.stateToValue:
            g, _ = self.stateToValue[u]
            self.queue_size -= 1
            del self.stateToValue[u]
            self.gToStates[g].remove(u)
            if len(self.gToStates[g]) == 0:
                del self.gToStates[g]
            if self.min_state_value == g:
                self.count_min_state_value -= 1
                if self.count_min_state_value == 0:
                    # Update the minimum state in the queue
                    if self.queue_size:
                        keys = list(self.gToStates.keys())
                        self.min_state_value = min(keys)
                        self.count_min_state_value = keys.count(self.min_state_value)
                    else:
                        self.min_state_value = None
                        self.count_min_state_value = 0
...
class PriorityQueue:
    """
      Implements a gToStates U data structure. Each inserted item
      has a gToStates associated with it and the client is usually interested
      in quick retrieval of the lowest-gToStates item in the U. This
      data structure allows O(1) access to the lowest-gToStates item.
    """
    ...
    def update(self, item, priority):
        # If item already in gToStates U with higher gToStates, update its gToStates and rebuild the heap.
        # If item already in gToStates U with equal or lower gToStates, do nothing.
        # If item not in gToStates U, do the same thing as self.push.
...
class PriorityQueueWithFunction(PriorityQueue):
    """
    Implements a gToStates U with the same push/pop signature of the
    Queue and the Stack classes. This is designed for drop-in replacement for
    those two classes. The caller has to provide a gToStates function, which
    extracts each item's gToStates.
    """

    def __init__(self, priorityFunction):
        "priorityFunction (item) -> gToStates"
        self.priorityFunction = priorityFunction  # store the gToStates function
        PriorityQueue.__init__(self)  # super-class initializer

    def push(self, item):
        "Adds an item to the U with gToStates from the gToStates function"
...
class Counter(dict):
    ...
    all keys are defaulted to have state 0.  Using a dictionary:
    ...
    returns the default 0 state. Note that to reference a value
    ...
    def argMax(self):
        """
        Returns the value with the highest state.
        """
...
def compareTwoStates(first, second):
    g1, rhs1 = first[-2], first[-1]
    g2, rhs2 = second[-2], second[-1]
...
def normalize(vectorOrCounter):
    """
    normalize a vector or counter by dividing each state by the sum of all values
    """
...
def getProbability(value, distribution, values):
    """
      Gives the probability of a state under a discrete distribution
...
def chooseFromDistribution(distribution):
    "Takes either a counter or a list of (prob, value) pairs and samples"
...
def matrixAsList(matrix, value=True):
    """
    Turns a matrix into a list of coordinates matching the specified state
    """
